...
import { Zap, ArrowRight, Mail, CheckCircle, Loader2, User } from 'lucide-react';
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4 relative overflow-hidden">
      ...
      >
        ...
        <div className="mb-6 space-y-3">
          ...
          <Button
            variant="ghost"
            className="w-full h-12 gap-3 border border-dashed border-border hover:border-primary/50 transition-all text-muted-foreground hover:text-foreground"
            onClick={async () => {
              setLoading(true);
              setError('');
              ...
              if (data.user) {
                // Auto-create a guest profile on the first campus
                const guestName = `Guest_${data.user.id.slice(0, 6)}`;
                await supabase.from('profiles').insert({
                  user_id: data.user.id,
                  campus_id: 'bec319a9-3dab-4c86-9873-af609d2e71bc',
                  username: guestName.toLowerCase(),
                  display_name: guestName,
                  bio: 'Just browsing 👀',
                  interest_tags: ['Exploring'],
                });
                await fetchProfile();
                navigate('/dashboard');
              }
